...
# {
	...
	# Invoke-RestMethod -Uri "https://wiki.ppfr.fr/attachments/20" -outfile "c:\temp\glpi\fusioninventory-agent_windows-x64_2.6.exe"
	# Invoke-RestMethod -Uri "https://wiki.ppfr.fr/attachments/22" -outfile "c:\temp\glpi\install-fusioninventoryagent1.ps1"
	# start-process powershell -Credential $cred -argumentlist '-noprofile -command &{start-process powershell -argument "c:\temp\glpi\install-fusioninventoryagent1.ps1" -verb runas}'
# }
# ELSE
# {
	# Invoke-RestMethod -Uri "https://wiki.ppfr.fr/attachments/20" -outfile "c:\temp\glpi\fusioninventory-agent_windows-x64_2.6.exe"
...
### Parameters
    # param(
		...
		$SetupOptions = "/acceptlicense /runnow /execmode=service /add-firewall-exception /tag=PPF /S /no-ssl-check"
		$GLPIUri = "https://glpi.ppfr.fr/glpi/plugins/fusioninventory"
    # )
...
### Function to define the address of the GLPI Server in the registry
    Function DefineGLPIServer{
        ...
        if((Get-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\FusionInventory-Agent\" -Name "Server" -ErrorAction SilentlyContinue) -and ($SetupArchitecture -eq "x86") -and ($ComputerArchitecture -eq "x64")){
             ...
             # Modify in the registry the GLPI server URI
             Set-ItemProperty -Path "HKLM:\SOFTWARE\FusionInventory-Agent\" -Name "Server" -Value $GLPIUri -errorAction SilentlyContinue

             # Restart "FusionInventory Agent" Service
             Restart-Service "FusionInventory Agent" -errorAction SilentlyContinue
